...
.qc{background:var(--bg2);border:1px solid var(--border);border-radius:7px;padding:10px 13px;margin-bottom:7px;cursor:pointer;transition:transform .15s,box-shadow .15s,border-color .15s}
.qc:hover{border-color:var(--accent);transform:translateY(-2px);box-shadow:0 8px 24px rgba(0,0,0,.4)}
...
.jmm{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:14px}
.jz{border-radius:10px;padding:11px 8px;text-align:center;font-size:.74rem;box-shadow:0 4px 16px rgba(0,0,0,.4),0 1px 3px rgba(0,0,0,.6),inset 0 1px 0 rgba(255,255,255,.06);transition:transform .2s,box-shadow .2s;cursor:default}
.jz:hover{transform:translateY(-5px) scale(1.04);box-shadow:0 12px 30px rgba(0,0,0,.55),inset 0 1px 0 rgba(255,255,255,.1)}
.jz-t{font-weight:700;margin-bottom:4px;font-size:.76rem;letter-spacing:.02em}
.jz-d{font-size:.63rem;color:var(--dim);line-height:1.45;opacity:.85}
.jz-sk{background:linear-gradient(135deg,#58a6ff22 0%,#58a6ff08 100%);border:1px solid #58a6ff55;color:#58a6ff}
.jz-hp{background:linear-gradient(135deg,#3fb95022 0%,#3fb95008 100%);border:1px solid #3fb95055;color:#3fb950}
.jz-ma{background:linear-gradient(135deg,#d2a8ff22 0%,#d2a8ff08 100%);border:1px solid #d2a8ff55;color:#d2a8ff}
.jz-pc{background:linear-gradient(135deg,#d2992222 0%,#d2992208 100%);border:1px solid #d2992255;color:#d29922}
.mgrid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:10px}
.mbox{background:linear-gradient(160deg,var(--bg3) 0%,#1a1f27 100%);border:1px solid var(--border);border-radius:8px;padding:11px;box-shadow:0 4px 15px rgba(0,0,0,.3),inset 0 1px 0 rgba(255,255,255,.04)}
.mbox-h{font-size:.68rem;text-transform:uppercase;letter-spacing:.07em;margin-bottom:8px;padding-bottom:6px;border-bottom:1px solid var(--border)}
.stk .mbox-h{color:#58a6ff}
.hp .mbox-h{color:#3fb950}
.sta .mbox-h{color:#d2a8ff}
.me{background:var(--bg4);border:1px solid var(--border);border-radius:4px;padding:4px 8px;margin-bottom:4px;font-size:.74rem;font-family:monospace;display:flex;justify-content:space-between;gap:6px;box-shadow:inset 0 1px 0 rgba(255,255,255,.03)}
...
.fsec-h{font-size:.7rem;text-transform:uppercase;letter-spacing:.08em;color:var(--accent);margin-bottom:10px;padding-bottom:6px;border-bottom:1px solid #58a6ff33;text-shadow:0 0 20px rgba(88,166,255,.25)}
.step{display:flex;gap:10px;margin-bottom:8px;align-items:flex-start}
.step-n{background:linear-gradient(135deg,#79c0ff,#1f6feb);color:#fff;border-radius:50%;width:24px;height:24px;font-size:.7rem;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:2px;box-shadow:0 2px 8px rgba(88,166,255,.5),0 0 0 2px rgba(88,166,255,.15)}
.step-t{background:linear-gradient(135deg,var(--bg3) 0%,#1c2128 100%);border:1px solid var(--border);border-left:3px solid var(--accent);border-radius:6px;padding:9px 13px;font-size:.8rem;flex:1;line-height:1.5;box-shadow:0 3px 10px rgba(0,0,0,.3),inset 0 1px 0 rgba(255,255,255,.04)}
.arr{color:var(--accent);font-size:.85rem;margin-left:11px;margin-bottom:2px;opacity:.5;text-align:left}
.tr{display:flex;gap:10px;margin-bottom:6px;align-items:flex-start}
.tr-n{background:linear-gradient(135deg,#56d364,#238636);color:#fff;border:none;border-radius:50%;width:22px;height:22px;font-size:.68rem;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:2px;box-shadow:0 2px 8px rgba(63,185,80,.45),0 0 0 2px rgba(63,185,80,.15)}
.tr-c{background:#0d1117;border:1px solid #3fb95033;border-left:3px solid #3fb950;border-radius:5px;padding:6px 10px;font-size:.76rem;font-family:'Cascadia Code','Consolas',monospace;flex:1;white-space:pre-wrap;word-break:break-all;box-shadow:0 2px 8px rgba(0,0,0,.4),inset 0 1px 0 rgba(63,185,80,.06)}
.pipeline{display:flex;align-items:center;overflow-x:auto;padding:8px 2px 14px;margin-bottom:12px;gap:0;scrollbar-width:thin}
.pipe-step{display:flex;align-items:center;flex-shrink:0}
.pipe-chip{padding:7px 10px;border-radius:7px;text-align:center;min-width:80px;max-width:110px;box-shadow:0 3px 12px rgba(0,0,0,.4),inset 0 1px 0 rgba(255,255,255,.07);transition:transform .15s,box-shadow .15s;cursor:default}
.pipe-chip:hover{transform:translateY(-4px);box-shadow:0 8px 22px rgba(0,0,0,.55),inset 0 1px 0 rgba(255,255,255,.12)}
.pipe-n{font-size:.59rem;margin-bottom:2px;opacity:.65;font-family:monospace}
...
function renderCards(){
  ...
  for(let i=startIdx;i<end;i++){
    ...
    d.className='qc';d.style.borderLeft='3px solid '+q.col+'88';d.onclick=()=>showDetail(q,i);
...
function tcColor(tc){
  const t=String(tc||'');
  if(t==='O(1)'||t.indexOf('constant')!==-1)return '#3fb950';
  if(t.indexOf('n log')!==-1||t.indexOf('n·log')!==-1)return '#ff9800';
  if(t.indexOf('log n')!==-1||t.indexOf('log N')!==-1)return '#58a6ff';
  if(t==='O(n)'||t==='O(N)')return '#ffc107';
  if(t.indexOf('n\u00b2')!==-1||t.indexOf('n^2')!==-1||t.indexOf('n*n')!==-1)return '#f85149';
  if(t.indexOf('2^n')!==-1||t.indexOf('exponential')!==-1||t.indexOf('n!')!==-1)return '#d2a8ff';
  return '#d29922';
}
function renderFlow(q){
  ...
  if(q.lang==='sql'){
    const SQL_PIPE=[
      {n:'01',t:'FROM',sub:'JOIN sources',col:'#00bcd4'},
      {n:'02',t:'WHERE',sub:'filter rows',col:'#2196f3'},
      {n:'03',t:'GROUP BY',sub:'aggregate',col:'#9c27b0'},
      {n:'04',t:'HAVING',sub:'filter groups',col:'#ff9800'},
      {n:'05',t:'SELECT',sub:'project cols',col:'#4caf50'},
      {n:'06',t:'DISTINCT',sub:'deduplicate',col:'#00e5ff'},
      {n:'07',t:'ORDER BY',sub:'sort rows',col:'#ffc107'},
      {n:'08',t:'LIMIT',sub:'paginate',col:'#f44336'}
    ];
    const pipeHtml='<div class="pipeline">'+SQL_PIPE.map(function(p,i){
      return '<div class="pipe-step">'+(i>0?'<span class="pipe-arr">&#8250;</span>':'')+
        '<div class="pipe-chip" style="background:linear-gradient(135deg,'+p.col+'28,'+p.col+'08);border:1px solid '+p.col+'55;color:'+p.col+'">'+
        '<div class="pipe-n">phase '+p.n+'</div>'+
        '<div class="pipe-t">'+p.t+'</div>'+
    ...
    document.getElementById('tp-flow').innerHTML=
      '<div class="fsec"><div class="fsec-h">&#128200; SQL Execution Pipeline</div>'+pipeHtml+'</div>'+
      '<div class="fsec"><div class="fsec-h">&#128204; Logical Execution Order</div>'+sqlSteps+'</div>'+extra+
